    ...
    for m in xrange(M):
        for n in xrange(N):
            ...
            for cell in cells:
                ...
                lx_cell = ((2.0 * c_x) / N ) - 1
                ly_cell = ((2.0 * c_y) / M ) - 1
                a_cell = c_x - floor(c_x) 
                b_cell = c_y - floor(c_y)
                ...
                ###### X velocity
                #v_h = V_tile[0]*(1-abs(lx_cell)) - ( (1-D_QCA) * (lx_cell) )
                #v_h = V_tile[0]*(D_QCA) - ( (1-D_QCA) * (lx_cell) )
                #v_h = ( (1-D_QCA) * (lx_cell) )
                #v_h = - ( (1-D_QCA) * (lx_cell) )
                ...
                ###### Y velocity
                #v_v = V_tile[1]*(1-abs(ly_cell)) - ( (1-D_QCA) * (ly_cell) )
                #v_v = V_tile[1]*(D_QCA) - ( (1-D_QCA) * (ly_cell) )
                #v_v = ( (1-D_QCA) * (ly_cell) )
                #v_v = - ( (1-D_QCA) * (ly_cell) )
...
def diffusion(Chimera, QCA, bins, tiles, configuration):
    ...
    while (diffuse and (i<100)):
        ...
        ########## Calculate tile velocity vectors
        #V = velocityVectors(D)
